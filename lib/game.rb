class Game
  ...
  def errors_made # 6
    errors.length # Получаем длину массива ошибочных букв 4
  end

  def errors_allowed # 5 Проверяем количество разрешенных ошибок
    TOTAL_ERRORS_ALLOWED - errors_made # Получаем количество попыток 7 - 4 = 3
  end
  ...
  def lost? # 4 Проверяем проиграна ли
    errors_allowed == 0 #Проиграно, если дозволено ошибок 0
  end

  def over? # 1
    won? || lost? # Игра будет закончена, если выграна или проиграна
  end
  ...
  def won? # 3 Проверяем выиграна ли
    (normalized_letters(@letters) - normalized_letters(@user_guesses)).empty? # Вычетаем из массива слова буквы пользователя
  end

  def word
    @letters.join # Собираем слово из массива
  end
